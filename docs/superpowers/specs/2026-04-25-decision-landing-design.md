# 회고 아카이브 — 3-ICP 랜딩 페이지 설계 문서

- **작성일**: 2026-04-25
- **목적**: "회고 아카이브" 아이디어의 ICP별 수요를 동시 검증하기 위한 3개 랜딩 페이지 설계
...
이전 검토에서 "회고 아카이브" 아이디어가 일반 ICP("결정을 자주 하는 사람")로는 너무 광범위해서 페인 정의가 약하다는 결론에 도달했다. 또한 인접 시장(Loqbooq 폐업, Limitless Meta 흡수, Woebot D2C 종료)이 보여주는 신호는 "AI 메모리 회고 D2C" 카테고리가 단독으로는 검증 어려움을 시사한다.
